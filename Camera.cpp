...
void CCamera::switchTo(Mode mode) {
    this->mode = mode;

    switch(mode) {
        case Mode::FirstPerson:
            offset = XMFLOAT3 { 0.0f, 1.2f, 0.0f };
            break;
        case Mode::ThirdPerson:
            offset = XMFLOAT3 { 0.0f, 3.0f, -6.0f };
            break;
    }
}
...
TankCamera::TankCamera() : CCamera { } {
    horizontal_max_angle = 50.0f;
    horizontal_min_angle = -10.0f;
    switchMode();
}


void TankCamera::switchMode() {
    CCamera::switchMode();
