...
public:
    HeightMap(const char* file_name, float x_length, float y_length, float z_length);
    ~HeightMap();
...
public:
    ...
    int GetHeightMapWidth() { return x_length; }
    int GetHeightMapLength() { return x_length; }

    int getXLength() { return x_length; }
    int getZLength() { return z_length; }

    CMesh* createMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList);
